...
# mutable struct Lattice{Matrix{Site}} end

function lattice(N::Int)::Matrix{Site}#::Lattice 
...
function sum_neighbors(lattice::Matrix{Site},i::Int,j::Int)::Int8 
    nr,nc = size(lattice)
...
function sum_neighbors(J::Real,lattice::Matrix{Site},i::Int,j::Int)::Int8
    # Summing the neighbors of i,j
    return -J*get_value(lattice[index(i),index(j)])*sum_neighbors(lattice,i,j)
end

function sum_neighbors(J::Real,lattice::Matrix{Site},i::Int,j::Int,flip=true)::Int8
    # Summing the neighbors of i,j, with i,j flipped
    return -J*get_value(!lattice[index(i),index(j)])*sum_neighbors(lattice,i,j)
end

function sum_neighbors(J::Real,lattice::Matrix{Site})::Real
    # Total energy
    nr,nc = size(lattice)
    S = Real(0)
...
function sum_neighbors(J::Real,lattice::Matrix{Site},flip=true)::Real
...
function flip!(lattice::Matrix{Site},i::Int,j::Int)
...
function evolve_state!(J::Real,β::Real,lattice::Matrix{Site},i::Int,j::Int,E_old::Float64)
